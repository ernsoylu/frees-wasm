...
| [`fixtures/README.md`](fixtures/README.md) | The parity harness: corpus (1271 — **Wave J** swept the 114 Java test classes that were never in the harvester's class list; +255; Wave A1 promoted 14 property holds, Wave C1 added CO2, and **Wave P2 probed the 19 Wave-J holds and promoted 18**) and golden fixtures, the `.tables.json` request-table sidecars (Wave I), the pending set (**6**; the `dyn_accessor_live` cost hold, the 4 Wave-I exact-zero documents A1 could not express as a relative tolerance, and P2's one whose binding variable the model does not determine), the decayed-signal measure (Wave G1) for ODE row cells and the `ode45-adaptive-path` mechanism (Wave P2), how to run the gate and why the single-package form refuses, tolerance policy (`fixtures/tolerances-rustprop.json` grades what ships; `fixtures/tolerances.json` describes the table configuration and nothing replays it today), oracle-established ground truths |
...
cargo test --release --workspace       # all tests incl. the parity replay
                                       # (--release: the replay solves 1271 documents,
                                       #  ~360 s of the run — Wave P2, 2026-08-24)
