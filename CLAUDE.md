...
| [`fixtures/README.md`](fixtures/README.md) | The parity harness: corpus (772) and golden fixtures, the pending set (5; one is a fluid-linkage hold), how to run the gate and why the single-package form refuses, tolerance policy (`fixtures/tolerances-rustprop.json` grades what ships; `fixtures/tolerances.json` describes the table configuration and nothing replays it today), oracle-established ground truths |
...
cargo test --release --workspace       # all tests incl. the parity replay
                                       # (--release: the replay solves 772 documents)
