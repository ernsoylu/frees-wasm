...
| [`fixtures/README.md`](fixtures/README.md) | The parity harness: corpus (1253 — **Wave J** swept the 114 Java test classes that were never in the harvester's class list; +255; Wave A1 promoted 14 property holds and Wave C1 added CO2) and golden fixtures, the `.tables.json` request-table sidecars (Wave I), the pending set (24; the `dyn_accessor_live` cost hold, the 4 Wave-I exact-zero documents A1 could not express as a relative tolerance, and 19 Wave-J property-chain / adaptive-path holds), the decayed-signal measure (Wave G1) for ODE row cells, how to run the gate and why the single-package form refuses, tolerance policy (`fixtures/tolerances-rustprop.json` grades what ships; `fixtures/tolerances.json` describes the table configuration and nothing replays it today), oracle-established ground truths |
...
cargo test --release --workspace       # all tests incl. the parity replay
                                       # (--release: the replay solves 1253 documents,
                                       #  ~353 s of the run — Wave J, 2026-08-24)
