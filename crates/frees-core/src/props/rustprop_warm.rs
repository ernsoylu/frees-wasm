...
///
/// **PENDING RUSTPROP SIGNATURE CHANGE.** A parallel rustprop task adds a
/// fourth parameter, the phase to impose:
///
/// ```text
///   today: solver_rho_tp_guessed(&self, t, p, rho_guess)               -> Result<f64>
///   after: solver_rho_tp_guessed(&self, t, p, rho_guess, phase: Phase) -> Result<f64>
/// ```
///
/// The new argument selects upstream's phase-imposed stability retries, and
/// `Phase::NotImposed` takes the catch-all `_ => Ok(rho)` arm — i.e. it is
/// **behaviour-identical to the three-argument form this adapter is
/// calibrated against**. When rustprop lands, pass
/// `rustprop::heos::...::Phase::NotImposed` here and nothing else in this
/// file changes.
fn solve_rho_tp_guessed(flash: &PtFlash, t: f64, p: f64, rho_guess: f64) -> Option<f64> {
    flash
        .solver_rho_tp_guessed(t, p, rho_guess)
        .ok()
