...
/// **Warm equals cold**, and warm is the more accurate of the two.
///
/// The comparison needs a word on tolerances. rustprop's cold `(P,X)` flash is
/// upstream's `HSU_P_flash`: a **bisection** in temperature stopped at a
/// `2^-29` relative bracket, whose midpoint it returns — so a cold `T` carries
/// up to `9.3e-10` relative granularity of its own. A density difference is
/// that temperature difference amplified by the state's own thermal expansion,
...
/// else would be asserting that cold is more precise than it is. The third
/// assertion is the one that actually says warm is right: it is at least as
/// close to the exact `(T,P)` state as cold is.
#[test]
fn warm_equals_cold_over_a_grid_across_four_fluids() {
    ...
    for x_key in ["Hmass", "Smass"] {
        for &(fluid, t, p) in GRID {
            ...
            // Warm is not merely close to cold, it is closer to the truth —
            // asked only where cold is a real flash, since for Air `t_cold`
            // *is* the truth and the comparison would be circular.
            if fluid != "Air" {
                let truth_warm = ((t_warm - t) / t).abs();
                let truth_cold = ((t_cold - t) / t).abs();
                assert!(
                    truth_warm <= truth_cold + 1.0e-14,
                    "{fluid} {x_key} T={t} p={p:e}: warm ({truth_warm:.3e} off the exact state) \
                     must be at least as accurate as cold ({truth_cold:.3e})"
                );
            }
    ...
    println!("warm vs cold over {checked} states: worst dT/T={worst_t:.3e} dD/D={worst_d:.3e}");
}
...
/// **Air.** rustprop serves the pseudo-pure fluids at `PT`/`QT`/`PQ` only —
/// `(P,Hmass)` is a loud `NotImplemented` there — and the adapter answers it
/// anyway, because its machinery *is* a sequence of `(T,p)` solves. The check
/// is a round trip through rustprop's own PT flash, which is the only oracle
/// there is for this pair today.
#[test]
fn air_p_hmass_and_p_smass_answer_through_pt_solves() {
    let _g = guard();
    assert!(
        rustprop::props_si("T", "P", 1.0e5, "Hmass", 4.0e5, "Air").is_err(),
        "the premise: rustprop itself still declines (P,Hmass) for Air"
    );
    for x_key in ["Hmass", "Smass"] {
    ...
    // a root stable with, so the adapter refuses it and rustprop's own message
    // reaches the caller.
    let h_cold = pt("Hmass", 100.0, 1.0e5, "Air");
    assert!(h_cold.is_finite());
    assert!(
        B.props_si("T", "P", 1.0e5, "Hmass", h_cold, "Air").is_err(),
        "sub-critical Air must stay refused, not guessed"
    );
}
