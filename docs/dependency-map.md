...
| `net.java.dev.jna:jna` | 5.19.1 | Apache-2.0 / LGPL-2.1 | Loading `libCoolProp.so`, SUNDIALS | Gone. Native builds use `extern "C"`; wasm uses either a JS import or an Emscripten-linked symbol (decision **D1/D2**). | ❌ |
...
|---|---|---|---|---|
| **CoolProp** | vendored `backend/core/native/libCoolProp.so`; upstream now **8.0.0** | MIT | Upstream ships an **official Emscripten build** (`coolprop.js` + `coolprop.wasm`, built with `-DCOOLPROP_JAVASCRIPT_MODULE=ON`). Binding surface is only 4 functions: `PropsSI`, `Props1SI`, `HAPropsSI`, `get_global_param_string`. Primary path is precomputed tables (D1-C) with this as the lazy fallback. | ⚠️ |
